...
def optimal_alignment(Matrix, s1, s2, top, bottom, i, j):
    ...
    if i>0 and Matrix[i][j] == Matrix[i-1][j] + 1:
        top.append(s1[i-1])
        bottom.append('|-|')
        return optimal_alignment(Matrix, s1, s2, top, bottom, i-1, j)

    elif j>0 and Matrix[i][j] == Matrix[i][j-1] + 1:
        bottom.append(f'|{s2[j-1]}|')
        top.append('|-|')
        return optimal_alignment(Matrix, s1, s2, top, bottom, i, j-1)

    else:
        top.append(f'|{s1[i-1]}|')
        bottom.append(f'|{s2[j-1]}|')
        return optimal_alignment(Matrix, s1, s2, top, bottom, i-1, j-1)


if __name__ == "__main__":
    s1, s2 = input(), input()
    edit_distance, Matrix = edit_distance(s1, s2)
